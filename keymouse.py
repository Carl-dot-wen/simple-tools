...
════════════════════════════════════════
修复内容：
  · 退出模式后程序不再意外退出（用 tkinter mainloop 替代 keyboard.wait）
  · 退出模式后修饰键不再粘连（_deactivate 时主动发送 release 事件）

新增功能：
  · 滚轮模式（上下左右键滚动页面，长按持续滚动）
  · 可视化 GUI：实时显示当前模式
  · 所有按键均可在界面内「录制」自定义
...
运行方式：
  python keyboard_mouse_gui.py
  （若快捷键无响应，请右键 → 以管理员身份运行）
